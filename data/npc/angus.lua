...
local function creatureSayCallback(npc, creature, type, message)
	...
	elseif MsgContains(message, "farmine") then
		if player:getStorageValue(Storage.Quest.TheNewFrontier.Questline) <= 15
		and player:getStorageValue(Storage.Quest.TheNewFrontier.BribeExplorerSociety) < 1 then
	...
	elseif MsgContains(message, "bluff") then
		if npcHandler:getTopic(playerId) == 30 then
			if player:getStorageValue(Storage.Quest.TheNewFrontier.BribeExplorerSociety) < 1 then
				npcHandler:say({
				...
				player:setStorageValue(Storage.Quest.TheNewFrontier.BribeExplorerSociety, 1)
				--Questlog, The New Frontier Quest "Mission 05: Getting Things Busy"
				player:setStorageValue(Storage.Quest.TheNewFrontier.Mission05, player:getStorageValue(Storage.Quest.TheNewFrontier.Mission05) + 1)
			end
		end
