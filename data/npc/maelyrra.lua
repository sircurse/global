...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "mission") then
		...
		elseif player:getStorageValue(Storage.Quest.ThreatenedDreams.Mission02[1]) >= 3
		and player:getStorageValue(Storage.Quest.ThreatenedDreams.Mission02[1]) <= 4 then
			npcHandler:say({
				"Have you found the moon mirror and freed the captured fairies?"
			}, npc, creature)
			npcHandler:setTopic(playerId, 4)
		elseif player:getStorageValue(Storage.Quest.ThreatenedDreams.Mission02[1]) == 5
		and player:getStorageValue(Storage.Quest.ThreatenedDreams.Mission03[1]) == 4 then
			npcHandler:say({
				"Have you already found the starlight vial and the sun catcher?"
			}, npc, creature)
			npcHandler:setTopic(playerId, 6)
		elseif player:getStorageValue(Storage.Quest.ThreatenedDreams.Mission02[1]) == 6 then
			npcHandler:say({
				"Could you already gather the three lights?"
			}, npc, creature)
			npcHandler:setTopic(playerId, 7)
		elseif player:getStorageValue(Storage.Quest.ThreatenedDreams.Mission02[1]) == 7 then
			npcHandler:say({
	...
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 1 then
			...
			player:setStorageValue(Storage.Quest.ThreatenedDreams.Mission02.EnfeebledCount, 0)
			player:setStorageValue(Storage.Quest.ThreatenedDreams.Mission02.FrazzlemawsCount, 0)
		...
		elseif npcHandler:getTopic(playerId) == 4 then
			if player:getStorageValue(Storage.Quest.ThreatenedDreams.Mission02.FairiesCounter) == 5
			and player:getStorageValue(Storage.Quest.ThreatenedDreams.Mission02.DarkMoomMirror) == 1 then
				npcHandler:say({
				...
				player:setStorageValue(Storage.Quest.ThreatenedDreams.Mission02[1], 4)
			elseif player:getStorageValue(Storage.Quest.ThreatenedDreams.Mission02[1]) == 4 then
				npcHandler:say({
		...
		elseif npcHandler:getTopic(playerId) == 5 then
			...
			player:setStorageValue(Storage.Quest.ThreatenedDreams.Mission02[1], 5)
			npcHandler:setTopic(playerId, 0)
		elseif npcHandler:getTopic(playerId) == 6 then
			...
			player:setStorageValue(Storage.Quest.ThreatenedDreams.Mission02[1], 6)
			npcHandler:setTopic(playerId, 0)
		...
		elseif npcHandler:getTopic(playerId) == 8 then
			if player:getStorageValue(Storage.Quest.ThreatenedDreams.Mission02.ChargedMoomMirror) == 0
			and player:getStorageValue(Storage.Quest.ThreatenedDreams.Mission02.ChargedStarlightVial) == 0
			and player:getStorageValue(Storage.Quest.ThreatenedDreams.Mission02.ChargedSunCacther) == 0 then
				npcHandler:say({
				...
				player:setStorageValue(Storage.Quest.ThreatenedDreams.Mission02[1], 8)
				npcHandler:setTopic(playerId, 0)
			else
				npcHandler:say({
					"If you charge all fifteen arcane sources with the respective light, Feyrist's protection will be ensured again."
