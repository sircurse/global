...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "farmine") then
		if player:getStorageValue(Storage.Quest.TheNewFrontier.Questline) == 15 then
	...
	elseif MsgContains(message, "impress") or MsgContains(message, "plea") then
		if npcHandler:getTopic(playerId) == 1 then
			if player:getStorageValue(Storage.Quest.TheNewFrontier.BribeLeeland) < 1 then
				...
				player:setStorageValue(Storage.Quest.TheNewFrontier.BribeLeeland, 1)
				player:setStorageValue(Storage.Quest.TheNewFrontier.Mission05, player:getStorageValue(Storage.Quest.TheNewFrontier.Mission05) + 1) --Questlog, The New Frontier Quest "Mission 05: Getting Things Busy"
			end
		end
	end
	return true
