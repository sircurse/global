	...
	if item.itemid == config.leverId then
		...
		for i = 1, #config.playerPositions do
			...
			if participant and participant:isPlayer() then
				...
				if config.daily and participant:getStorageValue(Storage.Quest.TheDreamCourts.FacelessBaneTime) > os.time() then
		...
		for i = 1, #team do
			...
			-- Assign boss timer
			team[i]:setStorageValue(Storage.Quest.TheDreamCourts.FacelessBaneTime, os.time() + config.timeToFightAgain * 60 * 60) -- 20 hours
