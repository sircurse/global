...
function theNewFrontierBeaver.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if target.actionid == 8002 then
		if player:getStorageValue(Storage.Quest.TheNewFrontier.Questline) == 5 and player:getStorageValue(Storage.Quest.TheNewFrontier.Beaver1) < 1 then
			Game.createMonster("thieving squirrel", toPosition)
			toPosition:sendMagicEffect(CONST_ME_TELEPORT)
			player:setStorageValue(Storage.Quest.TheNewFrontier.Beaver1, 1)
			player:setStorageValue(Storage.Quest.TheNewFrontier.Mission02, player:getStorageValue(Storage.Quest.TheNewFrontier.Mission02) + 1) --Questlog, The New Frontier Quest "Mission 02: From Kazordoon With Love"
			Game.createMonster("thieving squirrel", toPosition)
	...
	elseif target.actionid == 8003 then
		if player:getStorageValue(Storage.Quest.TheNewFrontier.Questline) == 5 and player:getStorageValue(Storage.Quest.TheNewFrontier.Beaver2) < 1 then
			for i = 1, 5 do
				pos = toPosition
	...
	elseif target.actionid == 8004 then
		if player:getStorageValue(Storage.Quest.TheNewFrontier.Questline) == 5 and player:getStorageValue(Storage.Quest.TheNewFrontier.Beaver3) < 1 then
			for i = 1, 3 do
				pos = toPosition
	...
	if player:getStorageValue(Storage.Quest.TheNewFrontier.Beaver1) == 1
	and player:getStorageValue(Storage.Quest.TheNewFrontier.Beaver2) == 1
	and player:getStorageValue(Storage.Quest.TheNewFrontier.Beaver3) == 1 then
